...
- (void)showMenuAtView:(UIView *)containerView {
    ...
    switch (self.menuAnimationType) {
        case kLLPopupMenusAnimationTypeNetEase:
...
- (void)showButtons {
    ...
    for (int index = 0; index < items.count; index ++) {
        ...
        LLMenuItem *menuItem = items[index];
        // 如果没有自定义index，就按照正常流程，从0开始
        if (menuItem.index < 0) {
            menuItem.index = index;
        }
        LLMenuButton *menuButton = (LLMenuButton *)[self viewWithTag:kMenuButtonBaseTag + index];
        ...
        switch (self.menuAnimationType) {
            case kLLPopupMenusAnimationTypeSina:
        ...
        if (!menuButton) {
            menuButton = [[LLMenuButton alloc] initWithFrame:fromRect menuItem:menuItem];
